        ...
        try {
            ...
            if (resultWriter != null) {
                ...
                if (resultReader != null) {
                    try { resultReader.close(); } catch (Exception e) { }
                }
            }
            if (resultFile != null) {
                resultFile.delete();
        ...
        try {
            reader =
                new BufferedReader(new InputStreamReader(
                    new FileInputStream(recordFile), "UTF-8"));
            ...
            if (content.indexOf("<header status=\"deleted\">") == -1) {
                ...
                throw new ServerException("Error reading from file: "
                    + xmlPath, e);
        ...
        while (newUserDefinedSetInfosIterator.hasNext()) {
            ...
            if (resourceIds != null) {
                try {
                    ...
                    _LOG.error("Adapt set membership for set spec " + curentSpec + " aborted."
                    );
        ...
        try {
            ...
            if (_db.isPollingEnabled(conn)) {
                ...
                while (currentUserDefinedSetSpecsIterator.hasNext()) {
                    String currentSetSpec = currentUserDefinedSetSpecsIterator.next();
                    if (!existSetSpecs.contains(currentSetSpec)) { 
                        this.newUserDefinedSetsMap.put(currentSetSpec, currentUserDefinedSets.get(currentSetSpec));
                    }
                }
              //update the existing user defined sets but not insert new user defined sets yet,
                //this will happen after update all records in cache in a separate step.
                //add new implicite sets and delete the altered sets.
                //update and remove, but don't add new sets (will happen later).
                ...
                if (latestRemoteDate > _db.getEarliestPollDate(conn)) {
                  ...
                  //Call updateFormats early, because if repository data not changed
                  //but some formats or their dissemination changed, we should update all
                  //existing records
                    
//                    checkImmediateShutdown();
//                    List allPrefixes = updateFormats(conn);
                    
                    //Call updateSets early, because if repository data is not changed
                    //but some user defined sets are removed from the set definitions database
                    //we also have to remove these sets and their set membership from cache 
...
//                    updateSets(conn);
    ...
    private void updateSets(Connection conn) throws Exception {
        ...
        try {
            while (riter.hasNext()) {
                 ...
                 */
              if (!this.newUserDefinedSetsMap.keySet().contains(encounteredSetSpec)) {
        ...
        for (String mdPrefix : allPrefixes) {
            ...
            if (lastPollDate < latestRemoteDate) {
                ...
                RemoteIterator<? extends Record> riter = _driver.listRecords(new Date(lastPollDate),
                    ...
                    mdPrefix,
                    this.newUserDefinedSetsMap.keySet());
    ...
    private QueueIterator newQueueIterator() throws Exception {
        ...
        try {
            ...
            if (queueWriter != null) {
                try { queueWriter.close(); } catch (Exception e) { }
            }
            if (queueFile != null) {
                queueFile.delete();
            }
