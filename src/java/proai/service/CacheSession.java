...
public class CacheSession<T> extends Thread
    ...
    public void run() {
        ...
        CloseableIterator<Vector<String>> iter = null;
        ...
        try {
            ...
            while (iter.hasNext() && !_threadNeedsToFinish) {
                ...
                for (int i = 0; i < incompleteListSize && iter.hasNext(); i++) {
                    Vector<String> pathAndDateAndSetSpecs = (Vector<String>) iter.next();
                    out.print(pathAndDateAndSetSpecs.remove(0));            // path
                    if (pathAndDateAndSetSpecs.size() > 0) {
                        out.print(" " + pathAndDateAndSetSpecs.remove(0));  // possibly date and set specs
                    Iterator<String> iterator = pathAndDateAndSetSpecs.iterator();
